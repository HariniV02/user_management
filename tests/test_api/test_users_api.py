...
@pytest.mark.asyncio
async def test_update_professional_status_not_allowed_test2(async_client, admin_user, user_token):
    headers = {"Authorization": f"Bearer {user_token}"}
    response = await async_client.patch(f"/users/{admin_user.id}/updateToProfessional", headers=headers)
    assert response.status_code == 403

@pytest.mark.asyncio
async def test_update_user_self_update_not_allowed_test3(async_client, admin_user, admin_token):
    updated_data = {"first_name": "Test", "last_name": "User"}
    response = await async_client.put("/users/updateProfile", json=updated_data)
    assert response.status_code == 401

@pytest.mark.asyncio
async def test_update_user_self_update_not_allowed_test4(async_client, verified_user):
    updated_data = {"first_name": "Test", "last_name": "User"}
    form_data = {
        "username": verified_user.email,
        "password": "MySuperPassword$1234"
    }
    login_response = await async_client.post("/login/", data=urlencode(form_data), headers={"Content-Type": "application/x-www-form-urlencoded"})
